...
const ANIMATION_PLAYED_KEY = "rewards_animation_played";
...
const RewardsSection = () => {
  ...
  const [pulsingBeans, setPulsingBeans] = useState(false);
  const [pulsingRewards, setPulsingRewards] = useState(false);

  const isAnimating = useRef(false);
  const rafBeans = useRef<number | null>(null);
  const rafRewards = useRef<number | null>(null);
  ...
  const triggerPulse = (setter: (v: boolean) => void) => {
    setter(false);
  ...
  const fetchData = async (withAnimation: boolean) => {
    ...
    try {
      ...
      if (withAnimation) {
        ...
        setDisplayBeans(0);
        setDisplayRewards(0);
        setPulsingBeans(false);
        setPulsingRewards(false);

        let beansDone = false;
        let rewardsDone = false;

        const checkDone = () => {
          if (beansDone && rewardsDone) isAnimating.current = false;
        };

        animateCount(balance, setDisplayBeans, rafBeans, () => {
          triggerPulse(setPulsingBeans);
          beansDone = true;
          checkDone();
        });

        animateCount(stampData.stampReward, setDisplayRewards, rafRewards, () => {
          triggerPulse(setPulsingRewards);
          rewardsDone = true;
          checkDone();
        });

        sessionStorage.setItem(ANIMATION_PLAYED_KEY, "1");
      } else {
        setDisplayBeans(balance);
        setDisplayRewards(stampData.stampReward);
      }
    } catch {
      setBeanBalance(0);
      setStampReward(0);
  ...
  useEffect(() => {
    const alreadyPlayed = sessionStorage.getItem(ANIMATION_PLAYED_KEY);
    fetchData(!alreadyPlayed);

    pollTimer.current = setInterval(() => fetchData(false), 2000);

    return () => {
      if (pollTimer.current) clearInterval(pollTimer.current);
      if (rafBeans.current) cancelAnimationFrame(rafBeans.current);
      if (rafRewards.current) cancelAnimationFrame(rafRewards.current);
    };
  }, []);
          ...
          <div className={styles.right}>
            ...
            <div className={styles.arrow} onClick={() => ionRouter.push("/rewards", "forward")}>
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M8 6L11 9L8 12" stroke="white" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </div>
          </div>
        </div>
