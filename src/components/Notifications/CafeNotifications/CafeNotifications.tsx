...
const PromoIcon = () => (
  <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M9.99935 1.6665C5.39935 1.6665 1.66602 5.39984 1.66602 9.99984C1.66602 14.5998 5.39935 18.3332 9.99935 18.3332C14.5993 18.3332 18.3327 14.5998 18.3327 9.99984C18.3327 5.39984 14.5993 1.6665 9.99935 1.6665ZM10.8327 14.1665H9.16602V9.1665H10.8327V14.1665ZM10.8327 7.49984H9.16602V5.83317H10.8327V7.49984Z" fill="#6C7A5F"/>
  </svg>
);
...
// ─── Push notification setup (FCM via Capacitor) ─────────────────────────────
...
async function setupPushNotifications(onForegroundPush?: () => void): Promise<PluginListenerHandle[] | void> {
  ...
  try {
    ...
    // Register for FCM (do not clear global listeners)
    await PushNotifications.register();

    const handles: PluginListenerHandle[] = [];

    // When FCM gives us a token, save it to the backend
    const regHandle = await PushNotifications.addListener("registration", async (token) => {
      try {
        ...
        if (!jwt) {
          console.warn("[FCM] No JWT present — cannot upload push token to backend (CafeNotifications)");
        }
        await registerFcmToken(jwt, token.value);
        await tokenStorage.setItem("push_token", token.value);
        console.log("[FCM] pushToken saved to backend");
      } catch (e) {
    ...
    // When a push arrives while the app is OPEN (foreground) — refresh the list
    const fgHandle = await PushNotifications.addListener("pushNotificationReceived", () => {
      console.log("[FCM] Push received in foreground — refreshing notifications");
      if (onForegroundPush) onForegroundPush();
    });
    handles.push(fgHandle);

    // When the user taps a notification (app may be backgrounded/closed), this event fires.
    const actionHandle = await PushNotifications.addListener("pushNotificationActionPerformed", (action) => {
      console.log("[FCM] Action performed on notification:", action);
      if (onForegroundPush) onForegroundPush();
    });
...
// We'll use the shared NoState component for notifications (bell variant)
...
const CafeNotifications: React.FC = () => {
  ...
  // Fetch in-app notifications on mount
  const fetchNotifications = useCallback(async () => {
    setLoading(true);
    try {
      const token = await tokenStorage.getToken();
      console.log("[CafeNotif] token present:", !!token);
      const all = await getNotifications(token);
      console.log("[CafeNotif] notifications received:", all.length, all);
      // Show all notifications (items have no individual timestamp, so we can't reliably date-filter)
  ...
  useEffect(() => {
    ...
    // Only set up push if user has explicitly enabled notifications in Settings
    let mounted = true;
    ...
    // Re-setup push handlers when app resumes (some Android devices lose listeners)
    ...
    // keep the appState listener handle so we can remove it cleanly
    (async () => {
      try {
        appStateHandle = await CapacitorApp.addListener("appStateChange", handleAppState);
      } catch {
        // older Capacitor may return Promise<void>; in that case the listener is registered
        // but we don't have a handle to remove — fallback to ignoring.
      }
    })();

    return () => {
      mounted = false;
      // Clean up only the listeners we added above
      (async () => {
        try {
          for (const h of pushHandles) {
        ...
        if (appStateHandle) {
          try { await appStateHandle.remove(); } catch { /* ignore */ }
        } else {
          // Older Capacitor versions may not return a handle when adding listeners.
          // In that case we must NOT call removeAllListeners() because it would
          // remove app-level listeners (like the handler in src/App.tsx) which
          // are expected to remain registered. So prefer leaving the listener
          // registered rather than clearing all app listeners.
          try { console.warn('[CafeNotif] appState listener not removable (no handle returned)'); } catch { /* ignore */ }
        }
      })();
    };
  ...
  return (
    <div className={styles.main}>
      <div className={styles.MainContainer}>
        ...
        <div className={
          !loading && notifications.length === 0 ? styles.centerEmpty : styles.BottomContainer
        }>
          {loading ? (
            // skeleton shimmer — reuses existing card structure
            [1, 2, 3].map((i) => (
            ...
            <NoState
              ...
              subtitle={"Browse the menu to brew something ☕"}
            />
          ) : (
