...
interface Props {
  onDeliveryModeChange?: (mode: "ship" | "pickup" | null) => void;
  onShippingEmirateChange?: (emirate: string) => void;
  onAddressReadyChange?: (ready: boolean) => void;
  onShippingAddressChange?: (address: Address | null) => void;
  onBillingAddressChange?: (address: Address | null) => void;
}

const Delivery = ({ onDeliveryModeChange, onShippingEmirateChange, onAddressReadyChange, onShippingAddressChange, onBillingAddressChange }: Props) => {
  // Start with no delivery mode selected — user must explicitly choose
  ...
  useEffect(() => {
    const ready =
      deliveryMode === "ship"
        ...
        ? !!billingAddress
        : false;
    onAddressReadyChange?.(ready);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [deliveryMode, shippingAddress, billingAddress]);
  ...
  const handleAddressSelect = (address: Address) => {
    if (sheetType === "shipping") {
      ...
      onShippingEmirateChange?.(address.emirates ?? "");
    } else {
      setBillingAddress(address);
      onBillingAddressChange?.(address);
    }
          ...
          <div className={styles.Top}>
            ...
            <div className={styles.ChooseCtas}>
              <button
                ...
                onClick={() => handleSetDeliveryMode("ship")}
              >
                Ship
              </button>
              ...
              <button
                ...
                onClick={() => handleSetDeliveryMode("pickup")}
              >
                Pick up
              </button>
            </div>
                ...
                <div className={styles.locationstore}>
                  ...
                  <h6>
                    <a
                      ...
                      style={{ color: '#637A57', textDecoration: 'underline' }}
                    >
                      Get Directions
                    </a>
                  </h6>
