...
const VegIcon = () => (
  <svg width="12" height="11" viewBox="0 0 12 11" fill="none">
    <path d="M0 10.3325V0H11.3632V10.3325H0ZM1.26258 9.1844H10.1007V1.14805H1.26258V9.1844ZM5.68162 7.46233C4.9872 7.46233 4.39274 7.2375 3.89822 6.78785C3.40371 6.33819 3.15646 5.79765 3.15646 5.16623C3.15646 4.5348 3.40371 3.99426 3.89822 3.54461C4.39274 3.09495 4.9872 2.87013 5.68162 2.87013C6.37604 2.87013 6.97051 3.09495 7.46502 3.54461C7.95953 3.99426 8.20679 4.5348 8.20679 5.16623C8.20679 5.79765 7.95953 6.33819 7.46502 6.78785C6.97051 7.2375 6.37604 7.46233 5.68162 7.46233Z" fill="#34A853" />
  </svg>
);
...
const VeganIcon = () => (
  <svg width="12" height="11" viewBox="0 0 12 11" fill="none">
    <path d="M6 0C3 2 2 5 2 7.5C2 9.4 3.6 11 6 11C8.4 11 10 9.4 10 7.5C10 5 9 2 6 0Z" fill="#0F9D58" />
  </svg>
);
...
const CafeMenu: React.FC = () => {
  ...
  const { cart, addToCart, decrementItem, setShopId: setCartShopId, refreshCart } = useCart();
  ...
  const guardCafeAdd = React.useCallback(
    (proceed: () => void) => {
      const storeHasItems = (storeCart?.items ?? []).length > 0 || storeCartCount > 0;
      if (storeHasItems) {
  ...
  const blockToastTimerRef = React.useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const triggerBlockToast = React.useCallback(() => {
    ...
    blockToastTimerRef.current = setTimeout(() => setShowBlockToast(false), 4000);
  }, []);
  ...
  useEffect(() => {
    ...
    const [repeatLastCustomizations, setRepeatLastCustomizations] = useState<any | null>(null);
      const [repeatProductName, setRepeatProductName] = useState<string | undefined>(undefined);
      const [repeatProductPrice, setRepeatProductPrice] = useState<number | undefined>(undefined);
      const [repeatProductSubtitle, setRepeatProductSubtitle] = useState<string | undefined>(undefined);
      const [repeatProductVegType, setRepeatProductVegType] = useState<VegType | undefined>(undefined);
      ...
      const _handleAddClick = async (item: MenuItem) => {
        if (hasActiveOrder) { triggerBlockToast(); return; }
        console.log("CafeMenu: add clicked", { shopId, productId: item.id });
        if (!shopId) {
          console.warn("CafeMenu: no shopId available, cannot open customization", { item });
          return;
        }
        ...
        if (existing) {
          console.log("CafeMenu: product exists in cart, opening RepeatCustomization", { productId: item.id, existing });
          setRepeatProductId(Number(item.id));
          setRepeatLastCustomizations((existing as any).customizations ?? (existing as any).customization ?? null);
        ...
        try {
          ...
          const cust = (detailed as any)?.customizations ?? (detailed as any)?.customization ?? [];
          const hasCustomizations = Array.isArray(cust) ? cust.length > 0 : Boolean(cust);

          if (!hasCustomizations) {
            try {
              await addToCart(Number((detailed as any).id), []);
              console.log("CafeMenu: added product directly to cart (no customizations)", { productId: detailed.id });
            } catch (err) {
          ...
          console.log("CafeMenu: opened customization for product", { productId: item.id });
        } catch (err) {
  ...
  const handleIncrementClick = async (entry: any) => {
    if (hasActiveOrder) { triggerBlockToast(); return; }
    console.log("CafeMenu: increment requested", { itemId: entry?.id, productId: entry?.productId ?? entry?.product?.id });
    try {
      ...
      const previousCust = (entry as any).customizations ?? (entry as any).customization ?? null;
      const menuCust = (foundItem as any)?.customizations ?? (foundItem as any)?.customization ?? [];
      const hasCustomizations = Array.isArray(menuCust) ? menuCust.length > 0 : Boolean(menuCust);

      if (!hasCustomizations) {
        console.log("CafeMenu: increment - product has no customizations, adding directly", { pid, entry });
        // Respect store-cart conflict guard
        guardCafeAdd(async () => {
          try {
            await addToCart(pid, previousCust ?? []);
          } catch (err) {
            console.error("CafeMenu: failed to add increment (no customizations)", err);
          }
        });
        return;
      }
      ...
      console.log("CafeMenu: opening RepeatCustomization for increment", { pid, entry });
      ...
      setRepeatProductSubtitle(foundItem?.tagline ?? foundItem?.desc ?? undefined);
  ...
  const handleDecrementClick = async (entry: any) => {
    console.log("CafeMenu: decrement requested", { itemId: entry?.id, productId: entry?.productId ?? entry?.product?.id });
    try {
  ...
  useEffect(() => {
    const loadMenu = async () => {
      try {
        ...
        console.log("🔒 [CafeMenu] shopClosed =", closed, "| isShopOpen field =", shop.isShopOpen);
        ...
        setMENU({
          BEVERAGES: markBestSellers(structuredMenu.BEVERAGES || []) as ApiMenuShape["BEVERAGES"],
          BAKERY: markBestSellers(structuredMenu.BAKERY || []) as ApiMenuShape["BAKERY"],
        });
    ...
    try {
      ...
      useEffect(() => {
        ...
        if (tab === "BEVERAGES" || tab === "BAKERY") {
          // set active tab state immediately
          setActiveTab(tab);
          const tryScroll = (attempt = 0) => {
            const el = document.querySelector(`[data-tab="${tab}"]`) as HTMLElement | null;
            if (el) {
              ...
              try {
                ...
                if (container) {
                  ...
                  container.scrollTo({ top: Math.max(0, relativeTop + container.scrollTop - 10), behavior: "smooth" });
                } else {
          ...
          // start trying shortly after mount
  ...
  // When menu finishes loading, if there are few items we still want the
  // scroll area to position so the last product is visible and roughly
  // centered. This runs once after menuLoaded toggles true and whenever
  // the MENU changes.
  useEffect(() => {
    if (!menuLoaded || !menuScrollRef.current) return;
    try {
      const container = menuScrollRef.current;
      const items = container.querySelectorAll(`.${styles.ItemCard}`);
      if (!items || items.length === 0) return;
      const last = items[items.length - 1] as HTMLElement;
      // compute target scroll so the last item is centered vertically
      const containerRect = container.getBoundingClientRect();
      const lastRect = last.getBoundingClientRect();
      const relativeTop = lastRect.top - containerRect.top;
      const target = container.scrollTop + relativeTop - container.clientHeight / 2 + last.clientHeight / 2;
      container.scrollTo({ top: Math.max(0, target), behavior: "smooth" });
    } catch (err) {
      // defensive: don't let scrolling errors break render
      console.warn("CafeMenu: auto-scroll failed", err);
    }
  }, [menuLoaded, MENU.BEVERAGES?.length, MENU.BAKERY?.length]);
  ...
  return (
    <IonPage>
      <IonContent fullscreen className="home-content">
        {!online ? (
          <OfflineOverlay />
        ) : (!menuLoaded || !cartPreloaded || wishlistLoading || !menuImagesPreloaded ? (
...
<path d="M21.8109 13.4114C21.8109 11.0306 19.8814 9.10016 17.5006 9.09997C15.1197 9.09997 13.1892 11.0304 13.1892 13.4114C13.1894 15.7921 15.1198 17.7217 17.5006 17.7217C19.8812 17.7215 21.8107 15.792 21.8109 13.4114ZM23.4109 13.4114C23.4107 16.6756 20.7649 19.3215 17.5006 19.3217C14.2361 19.3217 11.5894 16.6758 11.5892 13.4114C11.5892 10.1468 14.236 7.5 17.5006 7.5C20.765 7.50019 23.4109 10.1469 23.4109 13.4114Z" fill="white"/>
<path d="M24.8775 26.7006C24.8775 24.744 24.1007 22.8666 22.7171 21.483C21.3337 20.0996 19.457 19.3227 17.5006 19.3226C15.5439 19.3226 13.6665 20.0994 12.2829 21.483C10.8994 22.8666 10.1226 24.744 10.1226 26.7006C10.1224 27.1423 9.76427 27.5006 9.32257 27.5006C8.88086 27.5006 8.52277 27.1423 8.52258 26.7006C8.52258 24.3196 9.46807 22.0354 11.1517 20.3518C12.8353 18.6681 15.1195 17.7227 17.5006 17.7227C19.8814 17.7227 22.1648 18.6683 23.8484 20.3518C25.532 22.0354 26.4775 24.3196 26.4775 26.7006C26.4773 27.1423 26.1192 27.5006 25.6775 27.5006C25.236 27.5004 24.8777 27.1422 24.8775 26.7006Z" fill="white"/>
</svg>
                              ...
                              <React.Fragment key={index}>
                                ...
                                >
                                  ...
                                  <div className={styles.ItemCardRight}>
                                    <div className={styles.ItemCardRightTopvee}>
                                      <div className={styles.ItemCardRightTop}>
                                        <div className={styles.badges}>
                                          <div
                                            className={styles.VegandNonVegBadge}
                                          >
                                    ...
                                    <div
                                      className={styles.ItemCardRightBottomveee}
                                    >
                                      <div
  ...
  {(menuLoaded && cartPreloaded && !wishlistLoading) && (
                  ...
                  return (
                    <div key={cat.title}>
                      <div
                        ...
                        style={{ borderBottom: isLast ? "none" : "1px solid rgba(140,140,140,0.35)" }}
                        onClick={() => {
      ...
      {(cart?.items?.length ?? 0) > 0 && (storeCart?.items ?? []).length === 0 && (
        <ViewCart
          itemCount={cart?.items?.reduce((acc: number, item: any) => acc + (item?.quantity || 0), 0) ?? 0}
      ...
      />
        <RepeatCustomization
          ...
          onChoose={() => {
            ...
            (async () => {
              try {
                ...
                setCustomizingProduct(detailed);
                setIsCustomizationOpen(true);
                console.log("RepeatCustomization: user chose to open Customization", { productId: repeatProductId });
              } catch (err) {
                console.error("RepeatCustomization: failed to open customization", err);
              } finally {
                setIsRepeatOpen(false);
              }
            })();
          ...
          onRepeatLast={async (quantity = 1) => {
            ...
            try {
              console.log("RepeatCustomization: repeating last customization", { productId: repeatProductId, previous: repeatLastCustomizations, quantity });
      ...
      {showBlockToast && (
        <div className={toastStyles.toast}>
          ...
          <span className={toastStyles.text}>
            You already have a cafe order in progress. Please wait until it&apos;s completed.
          </span>
        </div>
      )}
    </IonPage>
